...
class itbit{
	...
	// var $api_url = 'https://beta-api.itbit.com/v1/';
	...
	public function funding_history($wallet_id){
		return $this->curl('wallets/'.$wallet_id.'/funding_history');
	}
	...
	public function create_order($wallet_id, $order_type, $amount, $price){
		$order_data = array('side' => ($order_type == 'sell' ? 'sell' : 'buy'),
							...
							"display" => (string)number_format($amount,4,'.',''),
							'price' => (string)$price,
							'instrument' => 'XBTUSD');
