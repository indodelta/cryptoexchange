   ...
   public static function fetchManageUserData($userId){
    ...
    //dd($data);
        ...
        if($data['addradio']==1 && $data['nationradio']==1){
            ...
            $mail = Mail::send('admin.email.account_verified',array('name'=>$user_da),function($message) use ($user_da){ $message->to($user_da->email, $user_da->username)->subject('Your PerfektPay Account has been Verified');
                             });                                 
        }
        ...
        if($data['addradio']!=$data['nationradio']){
           ...
           if($data['addradio']==2){  
               $user_da =  DB::table('users')->where(['id'=>$data['id']])->first(); 
               $mail = Mail::send('admin.email.account_varification_failed',array('name'=>$user_da,'type'=>'Address Poorf'),function($message) use ($user_da){ $message->to($user_da->email, $user_da->username)->subject('Your Account Verification Request was Rejected');
                    });  
            }else if($data['nationradio']==2){

               $user_da =  DB::table('users')->where(['id'=>$data['id']])->first();    
               $mail = Mail::send('admin.email.account_varification_failed',array('name'=>$user_da,'type'=>'National ID'),function($message) use ($user_da){ $message->to($user_da->email, $user_da->username)->subject('Your Account Verification Request was Rejected');
                    });
            }
        }

        if($data['addradio']==2 && $data['nationradio']==2){
                    
               $user_da =  DB::table('users')->where(['id'=>$data['id']])->first(); 
               $mail = Mail::send('admin.email.account_varification_failed',array('name'=>$user_da,'type'=>'National ID and Address Poorf'),function($message) use ($user_da){ $message->to($user_da->email, $user_da->username)->subject('Your Account Verification Request was Rejected');
                    });                
...
//     public static function userStatement($data){
...
//         $userData = DB::select('select `u`.`id` as `user_id`, `u`.`username`, `u`.`first_name`, `u`.`last_name`, `u`.`country_code`, `u`.`mobile_no`, 
// (select ct.name from countries as ct where u.country = ct.id) as country, `u`.`email`, `bnk`.`id` as `transaction_id`, `bnk`.`amount`, `bnk`.`type`,
// @b := round(@b + (CASE WHEN type=0 OR type=3 THEN amount ELSE 0 END) - (CASE WHEN type=1 OR type = 2 THEN amount ELSE 0 END),2) AS balance,
// `bnk`.`status`, `bnk`.`withdrawal_status`, "4" as side, `bnk`.`updated_at`, `bnk`.`created_by`, "bnk" as currency, `bnk`.`ref_id`, 
// (select o.limit_price from orders as o where o.id = bnk.ref_id) as limit_price, 
// (select o.avg_price from orders as o where o.id = bnk.ref_id) as avg_price,
// (select o.quantity from orders as o where o.id = bnk.ref_id) as quantity 
// FROM
//     (SELECT @b := 0.0) AS dummy 
//   CROSS JOIN
//     bank_transactions as `bnk`
// inner join `users` as `u` on `u`.`id` = `bnk`.`user_id` 
// where (`bnk`.`user_id` = "'.$data['userId'].'") 
// and (`bnk`.`status` = 1 or `bnk`.`status` = 5) 
// and (`bnk`.`type` = 0 or `bnk`.`type` = 1 or `bnk`.`type` = 2 or `bnk`.`type` = 3 or `bnk`.`type` = 4) 
// and (`bnk`.`withdrawal_status` = 0 or `bnk`.`withdrawal_status` = 2) 
// order by `bnk`.id asc');  

//         return $userData = array_reverse($userData);

//     }

    public static function userStatement($data){ //dd($data);
        
        if($data['sh_type'] ==1){ 

            $userData = DB::select('select s.id,s.ref_id,s.user_id,s.type,s.status,s.withdrawal_status, s.updated_at,s.limit_price,s.avg_price,s.quantity,s.currency,s.usd_amount as pusd,
 @b := round(@b + (CASE WHEN (type=0 && status=1) OR (type=3 && status=1)  THEN usd_amount ELSE 0 END) - (CASE WHEN (type=4 && status=5 && withdrawal_status=2) OR (type=2 && status=1 OR (type=1 && status=1)) THEN usd_amount ELSE 0 END),2) AS usd_blance,
@a := round(@a + (CASE WHEN type=0 OR (type = 2 && status = 1) THEN btc_amount ELSE 0 END) - (CASE WHEN type=1 OR (type=4 && status=5 && withdrawal_status=2) OR (type = 3 && status = 1) THEN btc_amount ELSE 0 END),4) AS btc_blance, "2" as sh_type, ref_amount_id
from 
(SELECT  @a := 0.0, @b := 0.0) AS dummy
CROSS JOIN
((select id,ref_id,user_id,type,status,withdrawal_status,amount as usd_amount,"0" as btc_amount,(select o.limit_price from orders as o where o.id = bnk.ref_id) as limit_price,(select o.avg_price from orders as o where o.id = bnk.ref_id) as avg_price,"0" as quantity, "usd" as currency ,updated_at, ref_amount_id from bank_transactions as bnk where status in (1,5) and type in (0,1,2,3,4) and withdrawal_status in(0,2) and user_id = '.$data['userId'].' order by updated_at)
            UNION
(select id,ref_id,user_id,type,status,withdrawal_status,"0" as usd_amount,amount as btc_amount,(select o.limit_price from orders as o where o.id = btc.ref_id) as limit_price,(select o.avg_price from orders as o where o.id = btc.ref_id) as avg_price,amount as quantity, "btc" as currency,updated_at, "0" as ref_amount_id from bitcoin_transactions as btc where status in (1,5) and type in (0,1,2,3,4) and withdrawal_status in(0,2) and user_id = '.$data['userId'].')
            order by updated_at) as s');  
        ...
        foreach ($userData as $key => $val) {  
            if($val->ref_id != 0 && ($val->type ==2 || $val->type ==3) && $val->status ==1 ){
                if($temp ==0){

                    $temp = $val->ref_id;   
                    $val->pusd += $userData[$key+1]->pusd;  
                    $val->quantity += $userData[$key+1]->quantity; 
                    $val->usd_blance = $userData[$key+1]->usd_blance; 
                    $val->btc_blance = $userData[$key+1]->btc_blance; 
        ...
        return array_reverse($userData2);
        ...
        }else if($data['sh_type'] ==2){



            $userData = DB::select('select s.id,s.ref_id,s.user_id,s.type,s.status,s.withdrawal_status, s.updated_at,s.limit_price,s.avg_price,s.quantity,s.currency,s.usd_amount as pusd,
 @b := round(@b + (CASE WHEN (type=0 && status=1) OR (type=3 && status=1)  THEN usd_amount ELSE 0 END) - (CASE WHEN (type=4 && status=5 && withdrawal_status=2) OR (type=2 && status=1 OR (type=1 && status=1)) THEN usd_amount ELSE 0 END),2) AS usd_blance,
@a := round(@a + (CASE WHEN type=0 OR (type = 2 && status = 1) THEN btc_amount ELSE 0 END) - (CASE WHEN type=1 OR (type=4 && status=5 && withdrawal_status=2) OR (type = 3 && status = 1) THEN btc_amount ELSE 0 END),4) AS btc_blance, "1" as sh_type, ref_amount_id
from 
(SELECT  @a := 0.0, @b := 0.0) AS dummy
CROSS JOIN
((select id,ref_id,user_id,type,status,withdrawal_status,amount as usd_amount,"0" as btc_amount,(select o.limit_price from orders as o where o.id = bnk.ref_id) as limit_price,(select o.avg_price from orders as o where o.id = bnk.ref_id) as avg_price,"0" as quantity, "usd" as currency ,updated_at, ref_amount_id from bank_transactions as bnk where status in (1,5) and type in (0,1,2,3,4) and withdrawal_status in(0,2) and user_id = '.$data['userId'].' order by updated_at)
